...
test_number="test1"
dir_in = ".\results\result_cause\cause"
...
def get_right_answer(list_path_in):
    ...
    :param list_path_in: the list of the path of the answer generated by Neural Network
    ...
    for path_in in list_path_in:
        ...
        number = path_in.split("\\")[5][-1]
        test_number = "dev"+number

        path_answer_bf = ".\data\Test_cause_network\\"+test_number+"\dev\\Contingency.Cause"
        with open(path_answer_bf, "r") as r_f:
            all = r_f.readlines()
            list_right_answer.extend(["0"]*len(all))
       # print(len(list_right_answer))

        path_answer_de = ".\data\Test_cause_network\\" + test_number + "\dev\\Contingency.Condition"
...
def eval(list_path_in, dict_ans):
    for path_in in list_path_in:
        ...
        print("%-20s\t%-20s\t%-20s\t%-20s\t" % ("Contingency.Cause", list_show_answer_b[0], list_show_answer_b[1], list_show_answer_b[2]))
        print("%-20s\t%-20s\t%-20s\t%-20s\t" % ("Contingency.Condition", list_show_answer_e[0], list_show_answer_e[1], list_show_answer_e[2]))
