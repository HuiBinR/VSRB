...
temp, dir_, a_de, file, time = sys.argv
a_de = float(a_de)

dir_in = dir_
dir_out = "/home/xzdong/rhb/Ding/score_dev_cause"+"/"+ file + "/" + str(a_de)
if os.path.exists(dir_out):
    shutil.rmtree(dir_out)
os.makedirs(dir_out)

path_result = "/home/xzdong/rhb/Ding/cause_dev"+"/"+ file + time
...
def computeMacroAvgPrf(dict_all_ground_ansList):
    ...
    print("%-25s\t%-20s\t%-20s\t%-20s\t" % ("Macro", avg_P, avg_R, macro_F))

    with open(path_result, "a", encoding="utf-8") as w_f:
        w_f.write(str(a_de) + " " + "%-20s\t%-20s\t%-20s\t%-20s\t" % ("Macro", avg_P, avg_R, macro_F))
        w_f.write("\n")
