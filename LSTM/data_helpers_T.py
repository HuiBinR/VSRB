...
def load_data_and_labels(train_data_file,label_1):
    ...
    for label in labels:
        if "Temporal.Beforeafter" in label:
            y.append([1, 0])
        else:
            y.append([0, 1])
...
def load_data(train_file, test_file,sequence_length ,label):
    """
...
#    x_explicit_arg1_padded,x_explicit_arg2_padded = pad_sentences(x_explicit_arg1, x_explicit_arg2,sequence_length)
    ...
    sentences_padded = x_train_arg1_padded + x_train_arg2_padded + x_test_arg1_padded +x_test_arg2_padded
