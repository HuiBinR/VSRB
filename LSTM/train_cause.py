...
import time
import datetime
import data_helpers_cause as data_helpers2
...
# Data loading params
outpath = './results/result_test_for_code'+"/"+ result + "/" + test
if os.path.exists(outpath) == False:
    os.makedirs(outpath)
tf.flags.DEFINE_float("dev_sample_percentage", .1, "Percentage of the training data to use for validation")
tf.flags.DEFINE_string("train_data_file","./data/Cause_train.txt","Data for Training")
tf.flags.DEFINE_string("test_data_file","./data/Test_cause_network/"+test+"/"+"dev"+"/"+test,"Data for Testing")
tf.flags.DEFINE_string("label","cause",'label for train')
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def train_step(x_arg1_batch,x_arg2_batch,y_batch):
            ...
            '''
            outs = sess.run([cnn.rnn_outputs1],feed_dict)
            outs = np.array(outs)
            print(str(outs.shape[0]) + '\t' + str(outs.shape[1]) + '\t' + str(outs.shape[2]))
            '''
            _, step, loss, accuracy, y_= sess.run(
                [train_op, global_step, cnn.loss, cnn.accuracy, cnn.input_y],
                feed_dict)
        ...
        def test_step(x_arg1_batch,x_arg2_batch, y_batch):
            ...
            step, loss, accuracy, precision, recall, prediction, y_= sess.run(
                [global_step, cnn.loss, cnn.accuracy,cnn.precision,cnn.recall,cnn.predictions, cnn.input_y],
                feed_dict)
