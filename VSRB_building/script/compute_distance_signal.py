...
def Search(queryFeat, queryList, allDataList):
	...
	dataSize = 745984  # image size     #### 这里需要改
	batchSize = 80000  # batch size
	...
	# extra data
	queryBatchSize = 3000   #query batch size
	...
	basePath = "/newVolume2/yaoliang/ImageSearch/feature_extraction/train_data_valid"  ### 这里需要改
	#batch search	
	for i in xrange(queryVec.shape[0]/queryBatchSize):
		...
		for j in xrange(dataSize/batchSize):
			values, indices = getSim(basePath, j, partQueryVec)
			if j==0:
				topVal = values
				topIndices = indices
			else:
...
def getSim(path, index, queryVec):
	...
	dataVec = None
	if index >= 10:
		dataVec = getFeature(path + str(index))
	else:
		dataVec = getFeature(path + "0" + str(index))
	...
	print "values: ", values
	indices += (index) * batchSize
	return values, indices[:,-topN:]
