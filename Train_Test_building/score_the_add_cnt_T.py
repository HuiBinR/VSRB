...
temp, a_de, result_num = sys.argv
a_de = float(a_de)

dir_in = "/home/xzdong/rhb/Ding/part_add_t"
dir_out = "/home/xzdong/rhb/Ding/score_part_add_t"+"/"+str(a_contingency)+"_"+str(b_expansion)
if os.path.exists(dir_out) == False:
    os.makedirs(dir_out)
path_result = "/home/xzdong/rhb/Ding/t"+"/"+result_num
                ...
                if len(list_num_other)>1:
                    ...
                    for r in list_rel_cnt:
                        ...
                        if "=" in r:
                            ...
                            cnt = r.split("=")[1]
                            score = 0
                            if "Temporal.Beforeafter" in rel_:
                                score = int(cnt) * dict_rel_score_single["all"] / dict_rel_score_single[rel_]
                            if "Temporal.DuringEquals" in rel_:
                                score = a_de * int(cnt) * dict_rel_score_single["all"] / dict_rel_score_single[rel_]
...
def write2file(dict_all_ground_ansList):
    dir_out = "C:\\Users\RHB\Desktop\\Ding_final\\"
    for rel in dict_all_ground_ansList.keys():
        path_out = dir_out + rel
        # print(path_out)
        list_ans = dict_all_ground_ansList[rel]
        with open(path_out, "a") as w_f:
            for ans in list_ans:
                w_f.write(ans)
                w_f.write("\n")
